...
namespace Server.Repositories
{
    public class SavedEGOInfoRepository(ServerDbContext ctx) : ISavedInfoRepository<SavedEGOInfo,SavedEgo>
    {
        ...
        public async Task<List<SavedEGOInfo>> GetMultiSaved(SearchSaveParams option)
        {
            ...
            query = _ctx.SavedEGOInfos.Where(e=>e.Name.Contains(option.Name, StringComparison.CurrentCultureIgnoreCase)
        ...
        {
            var foundSave = await _ctx.SavedEGOInfos.Where(e=>e.Id == id).Include(s=>s.SavedEgo).FirstOrDefaultAsync();
            if(foundSave!=null)
            {
